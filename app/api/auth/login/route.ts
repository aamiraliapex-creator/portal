...
export async function POST(req: Request) {
  try {
    ...
    const rows = await sql<U[]>`select id, name, email, password_hash, role, status, coalesce(session_version,0) as session_version from users where email = ${emailLc} limit 1`
    const user = rows[0]

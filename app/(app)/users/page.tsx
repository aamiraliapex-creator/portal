...
import { getCurrentUser, canManageUsers, canDeleteUsers, canManageRole } from '@/lib/authz'
...
export default async function Users() {
  ...
  const canDelete = canDeleteUsers(actor)
  return (
    <div>
      ...
      <div className="mt-5 overflow-x-auto rounded-xl border border-slate-200 bg-white">
        <table className="min-w-full divide-y divide-slate-100 text-sm">
          ...
          <tbody className="divide-y divide-slate-100">
            {rows.map((u) => {
              // A row's actions are only shown if the current actor is actually allowed to
              // manage that specific account — an ADMIN sees no Disable/Delete controls on
              // another ADMIN or a SUPER_ADMIN row, matching what the API would allow anyway.
              const canManageThisRow = canManage && !!actor && canManageRole(actor, u.role) && u.id !== actor.id
              return (
                <tr key={u.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3">{canManageThisRow ? <UserActions id={u.id} status={u.status} canDelete={canDelete} /> : null}</td>
                </tr>
              )
            })}
          </tbody>

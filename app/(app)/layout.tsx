...
const ADMIN: [string, string][] = [['Team & Users', '/users'], ['Audit Log', '/audit'], ['Settings', '/settings']]
    ...
    <div className="flex min-h-screen bg-slate-100">
      <aside className="hidden w-60 shrink-0 flex-col bg-ink-950 p-3 lg:flex">
        ...
        <nav className="flex-1 space-y-1 overflow-y-auto">
          {NAV.map(([label, href]) => <NavLink key={href} href={href} label={label} />)}
          <p className="px-3 pb-1 pt-4 text-[10px] font-semibold uppercase tracking-widest text-slate-500">Administration</p>
          {ADMIN.map(([label, href]) => <NavLink key={href} href={href} label={label} />)}
        </nav>
      </aside>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="flex h-16 shrink-0 items-center gap-4 border-b border-slate-200 bg-white px-4 sm:px-6">
          ...
          <div className="ml-auto flex items-center gap-3">
            <a href="/profile" className="text-right leading-tight hover:opacity-80">
            ...
            <div className="flex h-9 w-9 items-center justify-center rounded-full bg-ink-950 text-xs font-bold text-white">{session.name.split(' ').map((s) => s[0]).slice(0, 2).join('')}</div>
            <LogoutButton />
          </div>
        </header>

...
export default async function Payments({ searchParams: searchParamsInput }: { searchParams: Promise<{ tab?: string; from?: string; to?: string; status?: string }> }) {
  const scope = await getViewerScope()
  if (!scope || !scope.showMoney) return <NotAvailable note="Financial information is limited to billing and management roles." />
  const finScoped = !scope.allMoney
  ...
  const [tot] = await sql<{ paid: string; unpaid: string; cpaid: number; cunpaid: number }[]>`
    ...
    from payments p where (${finScoped} = false or exists (select 1 from customers fc where fc.id = p.customer_id and fc.agent_id = ${viewerId})) and ${inRange}`
  ...
  return (
    <div>
      <div className="flex items-center justify-between">
        ...
        {hasPermission(scope.user.role, 'payment.create') && <Link href="/payments/new" className="btn btn-red">+ Record payment</Link>}
      </div>
      ...
      <div className="mt-4 card overflow-x-auto">{await Panel({ tab, status, from, to, sql, finScoped, viewerId })}</div>
    </div>
  )
}

async function Panel({ tab, status, from, to, sql, finScoped, viewerId }: { tab: string; status: string; from: string | null; to: string | null; sql: ReturnType<typeof getSql>; finScoped: boolean; viewerId: string }) {
  ...
  if (tab === 'payments') {
    const rows = await sql<{ id: string; paid_at: string; kind: string; method: string; amount: string; status: string; invoice: string | null; customer_id: string; first_name: string; last_name: string }[]>`
      select p.id,p.paid_at,p.kind,p.method,p.amount,p.status,p.invoice,p.customer_id,c.first_name,c.last_name from payments p join customers c on c.id=p.customer_id where (${finScoped} = false or c.agent_id = ${viewerId}) and ${range} and ${stFilter} order by p.paid_at desc limit 300`
  ...
  if (tab === 'subscriptions') {
    const rows = await sql<{ id: string; first_name: string; last_name: string; legacy_member_id: string | null; plan: string | null; sub_status: string; next_payment: string | null }[]>`select id, first_name, last_name, legacy_member_id, plan, sub_status, next_payment from customers where (${finScoped} = false or agent_id = ${viewerId}) order by created_at desc`
  ...
  const rows = await sql<{ id: string; first_name: string; last_name: string; legacy_member_id: string | null; next_payment: string | null; paid: string; outstanding: string; last_paid: string | null }[]>`
    ...
    from customers c left join payments p on p.customer_id=c.id where (${finScoped} = false or c.agent_id = ${viewerId}) group by c.id order by paid desc`

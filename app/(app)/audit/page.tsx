  ...
  return (
    <div>
      ...
      <div className="mt-4 overflow-hidden rounded-xl border border-slate-200 bg-white">
        ...
        {rows.map((r, i) => (<div key={i} className="flex items-center gap-4 border-b border-slate-50 px-5 py-3"><span className="w-44 shrink-0 text-xs text-slate-500">{new Date(r.occurred_at).toLocaleString('en-US',{month:'short',day:'numeric',year:'numeric',hour:'numeric',minute:'2-digit'})}</span><span className="text-sm text-slate-700">{r.label}</span></div>))}
      </div>
    </div>
  )
}

...
import { isScopedToOwnWork, canSeeMoney } from '@/lib/authz'
...
export default async function Dashboard() {
  ...
  const { canSeeAllFinancialRecords } = await import('@/lib/authz')
  const finScoped = !me || !canSeeAllFinancialRecords(me.role)
  const sql = getSql()
  const [c] = await sql<{ total: number; active: number; newmonth: number }[]>`select count(*)::int total, count(*) filter (where sub_status='Active')::int active, count(*) filter (where coalesce(joined_at,created_at) >= date_trunc('month', now()))::int newmonth from customers where (${scoped} = false or agent_id = ${viewerId})`
  const [k] = await sql<{ total: number; open: number; needaction: number; active: number; waiting: number; resolved: number }[]>`select count(*)::int total, count(*) filter (where status not in ('Resolved','Dismissed'))::int open, count(*) filter (where status='Action Required')::int needaction, count(*) filter (where status in ('New','Action Required','Hearing Scheduled','Motion Prep','Under Review'))::int active, count(*) filter (where status='Waiting for Court')::int waiting, count(*) filter (where status in ('Resolved','Dismissed'))::int resolved from cases where (${scoped} = false or agent_id = ${viewerId})`
  const [t] = await sql<{ overdue: number }[]>`select count(*) filter (where status in ('Open','In Progress') and due_at < now())::int overdue from tasks where (${scoped} = false or assignee_id = ${viewerId})`
  const [p] = await sql<{ collected: string; outstanding: string; overdue_inv: number }[]>`select coalesce(sum(amount) filter (where status='Paid'),0) collected, coalesce(sum(amount) filter (where status in ('Pending','Overdue')),0) outstanding, count(*) filter (where status in ('Pending','Overdue'))::int overdue_inv from payments p where (${finScoped} = false or exists (select 1 from customers c where c.id = p.customer_id and c.agent_id = ${viewerId}))`
  const [h] = await sql<{ week: number; today: number }[]>`select count(*) filter (where next_action_at >= now() and next_action_at < now() + interval '7 days')::int week, count(*) filter (where next_action_at::date = now()::date)::int today from cases where (${scoped} = false or agent_id = ${viewerId})`
  const methods = await sql<{ method: string; total: string }[]>`select method, sum(amount) total from payments p where p.status='Paid' and (${finScoped} = false or exists (select 1 from customers c where c.id=p.customer_id and c.agent_id=${viewerId})) group by method order by total desc`
  const topAgents = await sql<{ name: string; n: number }[]>`select u.name, count(cu.*)::int n from users u left join customers cu on cu.agent_id=u.id where u.status='ACTIVE' and (${scoped} = false or u.id = ${viewerId}) group by u.name order by n desc limit 5`
  const recent = await sql<{ id: string; first_name: string; last_name: string; legacy_member_id: string | null; plan: string | null; sub_status: string; agent_name: string | null; pay_channel: string | null; cases: number; joined: string }[]>`select cu.id, cu.first_name, cu.last_name, cu.legacy_member_id, cu.plan, cu.sub_status, cu.pay_channel, coalesce(cu.joined_at,cu.created_at) joined, u.name agent_name, (select count(*) from cases k where k.customer_id=cu.id)::int cases from customers cu left join users u on u.id=cu.agent_id where (${scoped} = false or cu.agent_id = ${viewerId}) order by coalesce(cu.joined_at,cu.created_at) desc limit 5`
  ...
  const kpis: [string, string | number, string, string][] = [
    ...
    ...(showMoney ? [['Outstanding $', money(Number(p.outstanding)), 'text-brand-600', `${p.overdue_inv} overdue`] as [string, string, string, string]] : []),
  ]
  return (
    <div>
      {/* Holiday bar */}
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-3">
        {showMoney && (<div className="card p-5 lg:col-span-2">
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-3">
        {showMoney && (<div className="card lg:col-span-2">
      ...
      <div className="card mt-4">
        ...
        <div className="overflow-x-auto"><table className="min-w-full"><thead><tr><th>Member</th><th>Joined</th><th>Plan</th><th>Agent</th><th>Pay</th><th>Subscription</th><th>Cases</th></tr></thead><tbody>
          ...
          {recent.map((r) => <tr key={r.id} className="rowlink"><td><Link className="font-medium text-brand-600" href={`/customers/${r.id}`}>{r.first_name} {r.last_name}</Link><div className="text-xs text-slate-400">{r.legacy_member_id || ''}</div></td><td>{d(r.joined)}</td><td>{r.plan || '—'}</td><td>{r.agent_name || '—'}</td><td>{showMoney ? (r.pay_channel || '—') : '—'}</td><td className={subBadge(r.sub_status)}>● {r.sub_status}</td><td>{r.cases}</td></tr>)}
        </tbody></table></div>
      </div>

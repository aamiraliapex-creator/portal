...
import { isScopedToOwnWork, hasPermission } from '@/lib/authz'
...
type Row = {
  ...
  status: string; cdl: string; cmv: string; approval_status: string; next_action: string | null; next_action_at: string | null;
...
export default async function Cases({ searchParams: searchParamsInput }: { searchParams: Promise<{ q?: string }> }) {
  ...
  const rows = await sql<Row[]>`
    select k.id, k.citation, k.official_no, k.court, k.court_phone, k.state, k.status, k.cdl, k.cmv, k.next_action, k.next_action_at, coalesce(k.approval_status,'ACTIVE') as approval_status,
    ...
    where (${scoped} = false or k.agent_id = ${viewerId})
      and (${q} = '' or (coalesce(k.citation,'')||' '||coalesce(k.official_no,'')||' '||coalesce(k.court,'')||' '||c.first_name||' '||c.last_name) ilike ${'%' + q + '%'})
  ...
  return (
    <div>
      <div className="flex items-center justify-between">
        ...
        {hasPermission(me?.role ?? '', 'case.create') && <Link href="/cases/new" className="btn btn-red">+ New case</Link>}
      </div>
          ...
          <tbody>
            ...
            {rows.map((k) => (
              <tr key={k.id} className="rowlink">
                ...
                <td><span className={'badge ' + badge(k.status)}><span className={'dot ' + dotColor(k.status)} />{k.status}</span>{k.approval_status !== 'ACTIVE' && <span className={'badge ml-1 ' + (k.approval_status === 'PENDING' ? 'bg-amber-50 text-amber-700' : 'bg-rose-50 text-rose-700')}>{k.approval_status === 'PENDING' ? 'Pending approval' : 'Rejected'}</span>}</td>
